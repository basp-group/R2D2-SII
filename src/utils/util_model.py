...
def get_DNNs(num_iter: int, ckpt_path: pathlib.Path, device=None):
    ...
    for i in range(num_iter):
        ...
        model_loaded = torch_load(dnn[0], torch.device("cpu"))
...
def load_net(net, N, args, dnn_dict):
    ...
    if args.layers == 1:
        ...
        for i in range(args.layers):
            net_tmp = {
                f.split(f"unet{i+1}.")[1]: state_dict[f] for f in state_dict if f.startswith(f"unet{i+1}.")
            }
...
def create_net(args, device):
    ...
    if args.resume or "test" in args.mode:
        ...
        state_dict = torch_load(args.checkpoint, device)["state_dict"]
        load_dict = True
    if args.layers == 1:
        unet = UnetModel(
            in_chans=2, out_chans=1, chans=args.num_chans, num_pool_layers=args.num_pools, drop_prob=0.0
        ).to(device)
        if load_dict:
            net_tmp = {f.split(f"unet.")[1]: state_dict[f] for f in state_dict if f.startswith(f"unet.")}
        ...
        for i in range(args.layers):
            ...
            if load_dict:
                net_tmp = {
                    f.split(f"unet{i+1}.")[1]: state_dict[f]
                    for f in state_dict
                    if f.startswith(f"unet{i+1}.")
                }
                unet.load_state_dict(net_tmp)
